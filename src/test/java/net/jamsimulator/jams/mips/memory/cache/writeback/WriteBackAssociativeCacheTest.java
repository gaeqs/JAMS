...
class WriteBackAssociativeCacheTest {
	...
	void bytes(CacheReplacementPolicy policy) {
		...
		WriteBackAssociativeCache cache = new WriteBackAssociativeCache(memory, 4, 4, policy);
	...
	void testSimpleProblem() {
		...
		Cache mem = new WriteBackAssociativeCache(new MIPS32Memory(), 4, 8, CacheReplacementPolicy.LRU);
		...
		assertEquals(25104, stats.getHits());
		assertEquals(8304, stats.getMisses());
