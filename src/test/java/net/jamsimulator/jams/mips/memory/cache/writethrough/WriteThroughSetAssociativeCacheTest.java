...
class WriteThroughSetAssociativeCacheTest {
	...
	void bytes(CacheReplacementPolicy policy) {
		...
		WriteThroughSetAssociativeCache cache = new WriteThroughSetAssociativeCache(memory, 4, 4, 2, policy);
	...
	void testSimpleProblem() {
		...
		Cache mem = new WriteThroughSetAssociativeCache(new MIPS32Memory(), 4, 8, 2, CacheReplacementPolicy.LRU);
