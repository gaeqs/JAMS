...
public class PseudoInstructionBcL extends PseudoInstruction {
	...
	public CompiledInstruction[] compile(InstructionSet set, int address, ParameterParseResult[] parameters) {
		...
		if (!(bc instanceof BasicInstruction))
			throw new CompilerException("Basic instruction '" + InstructionBc.MNEMONIC + "' not found.");

		ParameterParseResult[] newParameters = new ParameterParseResult[]{
				ParameterParseResult.builder().immediate(offset).build()
		};

		return new CompiledInstruction[]{((BasicInstruction) bc).compileBasic(newParameters, this)};
	}
}
