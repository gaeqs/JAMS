...
public class PseudoInstructionBalcL extends PseudoInstruction {
	...
	public CompiledInstruction[] compile(InstructionSet set, int address, ParameterParseResult[] parameters) {
		...
		if (!(balc instanceof BasicInstruction))
			throw new CompilerException("Basic instruction '" + InstructionBalc.MNEMONIC + "' not found.");

		ParameterParseResult[] newParameters = new ParameterParseResult[]{
				ParameterParseResult.builder().immediate(offset).build()
		};

		return new CompiledInstruction[]{((BasicInstruction) balc).compileBasic(newParameters, this)};
	}
}
